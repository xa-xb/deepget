...
import { createPage } from '../router/createNode'
...
export const useUserStore = defineStore('user', {
     ...
     */
            ...
            const recurse = (items: any[], level: number = 1) => {
                ...
                for (const item of items) {
                    ...
                    if (level === 1) {
                        ...
                        */
                        if (item.page == undefined || item.path == undefined) continue;
                        const path: string[] = item.page.split("/")
                        obj.component = null
                        switch (path.length) {
                            case 1: obj.component = createPage(() => import(`@/views/${path[0]}.vue`))
                                break;
                            case 2: obj.component = createPage(() => import(`@/views/${path[0]}/${path[1]}.vue`))
                                break;
                            case 3: obj.component = createPage(() => import(`@/views/${path[0]}/${path[1]}/${path[2]}.vue`))
                                break;
                        }
                        if (obj.component === null) {
                            throw new Error('component error: ' + item.page)
                        }
                    }
                    result.push(obj)
                }
        ...
        signIn(loginParam: SignInRequestDto) {
            return new Promise((resolve, reject) => {
                mainApi.signIn(loginParam)
                    .then(res => {
