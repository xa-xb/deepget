...
router.beforeEach(async (to, _from, next) => {
  ...
  if (needInit) {
    ...
    let result: any = await mainApi.getProfile()
    if (result.data) {
      ...
      if (['', '/', '/sign_in'].includes(appStore.currentPath)) router.replace('/home')
  ...
  if (userStore.name && userStore.name.length > 0) {
    next()
  } else if (whiteList.includes(to.path)) {
    next()
  } else {
    next("/sign_in"); // 全部重定向到登录页
  }
});
